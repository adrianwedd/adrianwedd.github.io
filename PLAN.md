...
| `classify-inbox.mjs`     | For every file in `content/inbox/`, call LLM → `{section,tags}`; move file accordingly. Confidence < 0.8 ⇒ move to `untagged/`. Uses `.lock` files to prevent concurrent processing and caches results by file hash. | Before build step     |
